...
This work is supported by the "PARK3" (Förderkennzeichen: 03EE1104), "ReliaREN-Pro" (Förderkennzeichen: 03EI4052B), and "PV-Scan" (Förderkennzeichen: 0325588B) projects, and by [Solarzentrum Stuttgart GmbH](https://www.solarzentrum-stuttgart.com/de/) and Kiwa PI Berlin AG companies by providing the data.
...
The script uses Pix2PixHD generator models downloaded to the `~/.imgtr_models` directory. The `imgtr` generates a binary grid image with those models. The corner coordinates are computed from a binary grid by finding intersections of extended lines of the probabilistic Hough-line transform. The resulting dual graph faces are filtered for outliers, using face properties such as area, minimum parallelogram side ratio, area ratio with parallelogram, and perimeter.
...
Complete missing cells, assuming each component has a regular and rectangular cell structure. `imgtr_complete_cells` estimates the camera model from the identified cell coordinates, and reconstruction error is computed. If the assumption that the discovered cells are not the same size is not fulfilled, then the reconstruction error will be significant. In particular, if the reconstruction error is larger than `-E` (default -1 and value is ignored), the `imgtr_complete_cells` prints an error message for the considered image. The argument `-E` depends on the original image resolution.

The `-d` argument controls the camera model being used. `-d no_camera` switches of camera distortion model, and a pinhole camera model is assumed. Among other options are: `flexible`, `prism`, `rational`, see more options in `imgtr_complete_cells --help`. The camera model can be estimated from several images at once by using `-c` (or `--chunk`) argument. It is up to you to ensure that all images in every chunk come from the same camera.
...
The `imgtr_rd` script may be useful for situations, when traditional methods in camera distortion estimation is difficult, for example in IR drone inspections.
